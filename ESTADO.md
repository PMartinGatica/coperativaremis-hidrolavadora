...
> Última actualización: 2026-09-13 · HEAD `05a539e` (+ commits pendientes de esta sesión)
> · detalle completo en `ADR.md` (014–036)
...
2. **PIN de 4 dígitos por patente construido y verificado en vivo (ADR-036).** Pedido real
   del cliente (Gaby, cooperativa) en una llamada — protege la tarifa remis/socio de que
   cualquiera use una patente ajena. Grandfather clause: patentes sin PIN siguen como
   siempre. 87 tests verdes (84 + 3 nuevos), `tsc` limpio, verificación Edge headless real
   (cliente y admin) contra el build de producción.
3. **Falta un solo paso para que `hidro-api.insolvadev.com` muestre el producto completo:**
   que Pablo cree la app en Coolify (`docs/deploy-coolify.md`, sin acceso propio, ADR-035).
   Hoy ese link sigue en 404 — la app nunca se creó (ADR-021).
...
la app en Coolify. Tarifa remis/socio ahora protegida con PIN por patente (ADR-036), pedido real
del cliente tras ver el producto funcionando.
...
- `drizzle-kit generate` desincronizado (0000/0001 sin snapshots) — no afecta producción
  (runner propio no lo usa) pero repetirá el problema al próximo cambio de schema
  (TODOS.md, 2026-09-13).
