...
|---|---|---|
| Contratos compartidos | `packages/shared/src/` | **`permissions.ts` (matriz de roles: la usan la API y el panel, ADR-062/063)**, `types.ts` (DTOs + protocolo de dispositivo), `constants.ts` (tarifas, TTLs, umbrales), `validation.ts` (zod + `normalizePlate`), `errors.ts` (código → HTTP) |
...
| API — dominio | `apps/api/src/services/` | `sessionService.ts` (checkout + límite diario), `paymentService.ts` (**`processApproval` = punto único de aprobación**), `deviceService.ts` (heartbeat, autorización, pulsador), `machineService.ts` (tarifa por patente, disponibilidad), `transition.ts`, `settingsService.ts`, `adminService.ts` (login + JWT `sub`=id/`tv`), **`userService.ts` (cuentas del panel: reglas de último admin, autoedición, técnico intocable)** · auth en `auth/adminAuth.ts` (`requireAdmin` consulta la base en cada pedido, `requirePermission`) |
...
| Web — admin | `apps/web/src/admin/` | `AdminApp.tsx` + `session.tsx` (`useSession().can(permiso)`) + `userRules.ts` + `pages/` (Dashboard, Machines, Vehicles, Sessions, Payments, Logs, Settings, **Users, Account**) |
...
| Tests | `apps/api/tests/` | `e2e.test.ts`, `concurrency.test.ts`, `payment-flow.test.ts`, `plates.test.ts`, `device-security.test.ts`, `offline.test.ts`, `simulator-resilience.test.ts`, `admin.test.ts`, `reconciliation.test.ts`, `config-guards.test.ts`, `demo-mode.test.ts` (MODO DEMO en producción, ADR-048), **`webhook-routes.test.ts`** (lo que contesta el endpoint de webhook a cada vía de MP, ADR-052 — la primera suite que le pega de verdad), **`https-redirect.test.ts`** (C3/ADR-053: cuándo redirige a https y sobre todo cuándo NO, que es lo que evita romper al ESP32 y entrar en bucle), **`mesa-de-entrada.test.ts`** (C1: todo por el login real; la cuenta técnica no destraba, el operador sí, la baja corta en el acto), **`roles.test.ts`** (matriz ruta × rol, usuarios, clave, migración 0003), `helpers.ts` (`createPanelUser` da de alta por la puerta real) · web: `apps/web/tests/` (`roles.test.tsx`: client 401/403, menú, solo lectura) |
| Diseños de fase | `docs/designs/` | `reconciliacion-pagos.md`, `reconciliacion-pagos-ui.md`, `pin-patente-remis-socio.md`, `guardas-produccion-seed.md`, `deploy-web-estatico.md`, **`webhook-ipn-legado.md`** (ADR-052: incluye el review completo de las dos voces), **`roles-usuarios.md`** (ADR-062/063) |
...
- Migraciones: `apps/api/drizzle/` (`0000_init.sql` … `0003_admin_roles.sql`), aplicadas por un runner
